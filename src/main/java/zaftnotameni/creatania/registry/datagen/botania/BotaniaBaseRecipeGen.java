...
public class BotaniaBaseRecipeGen {
  ...
  public static JsonObject jinputItem(JsonObject root, ItemStack stack) {
    var in = empty();
    in.addProperty("item", ForgeRegistries.ITEMS.getKey(stack.getItem()).toString());
  ...
  public static JsonObject jfluidResult(JsonObject root, String resultName, String innerName, FluidStack stack) {
    var res = empty();
    res.addProperty(innerName, ForgeRegistries.FLUIDS.getKey(stack.getFluid()).toString());
  ...
  public static JsonObject jitemResult(JsonObject root, String resultName, String innerName, ItemStack stack) {
    var res = empty();
    res.addProperty(innerName, ForgeRegistries.ITEMS.getKey(stack.getItem()).toString());
  ...
  public static class Builder {
    ...
    public Builder saveAs(ResourceLocation id, String suffix, CachedOutput pCache) {
      try {
        DataProvider.saveStable(pCache, root, getPath(generator.getOutputFolder(), typeName, id, suffix));
      } catch (IOException e) {
        throw new RuntimeException(e);
      }
      return this;
    }
    public Builder saveAs(ResourceLocation id, CachedOutput pCache) {
