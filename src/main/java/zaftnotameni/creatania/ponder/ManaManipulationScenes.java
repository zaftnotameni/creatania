...
public class ManaManipulationScenes {
  ...
  private static void setupInitialMixerScene(CreataniaPonderUtils x, XYZ blaze, XYZ basin, XYZ mixer) {
    ...
    x.idleAfter(20, x.scene::showBasePlate).get();
    x.idleAfter(20, blaze.appearTo(EAST)).get();
    x.idleAfter(20, basin.appearTo(WEST)).get();
    x.idleAfter(20, mixer.appearTo(UP)).get();
  }
  ...
  private static void addManaBlocksAsIngredients( CreataniaPonderUtils x, ResourceLocation blockId,  XYZ basin) {
    ...
    x.scene.idle(60);
    x.idleAfter(20, x.dropBlocksOnto(basin, blockId, 4, i -> x.scene.idle(20))).get();
  }

  private static void drainFluidMana(CreataniaPonderUtils x, ResourceLocation fluidId, XYZ basin, XYZ drain) {
    drain.setBlock(AllBlocks.ITEM_DRAIN.getDefaultState()).run();
    x.idleAfter(1,drain.appearTo(EAST)).get();
    x.idleAfter(1,basin.modifyBlock(x.facing(BasinBlock.FACING, WEST))).get();
    x.idleAfter(1,basin.modifyNbt(BasinTileEntity.class, x.emptyTanksFn("OutputTanks"))).get();
    x.idleAfter(1,drain.modifyNbt(ItemDrainTileEntity.class, x.fillTanksFn("Tanks", fluidId, 1000))).get();
  }

  private static void processManaBlocks( CreataniaPonderUtils x, ResourceLocation fluidId, XYZ basin, XYZ mixer) {
    mixer.modifyNbt( MechanicalMixerTileEntity.class, x.startProcessingFn()).run();
    basin.modifyNbt( BasinTileEntity.class, x.fillTanksFn("OutputTanks", fluidId, 1000)).run();
    x.scene.idle(10);
    basin.modifyNbt( BasinTileEntity.class, x.emptyInputItemsFn()).run();
    x.scene.idle(100);
  }
  ...
  private static void superheatBlazeBurner(  CreataniaPonderUtils x, XYZ blaze) {
    ...
    x.showInputItem(AllItems.BLAZE_CAKE.get(), Pointing.UP, blaze.center(), 30, InputWindowElement::rightClick).run();
    x.scene.idle(30);
