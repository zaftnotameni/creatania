...
public class CobblegenRecipeCategory extends CreataniaRecipeProcessingCategory<CobblegenRecipe> {
  ...
  public CobblegenRecipeCategory(IGuiHelper helper) {
    // TODO: super(helper, helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(Blocks.REAL_MANA_BLOCK.get())));
    super(helper, helper.createDrawableIngredient(VanillaTypes.ITEM_STACK, ItemStack.EMPTY));
  }
  ...
  public Component getTitle() {
    return Component.literal("Mysterious Cobblegen");
  }
  ...
  public RecipeType<CobblegenRecipe> getRecipeType() {
    return CobblegenRecipe.TYPE.asJeiRecipeType();
  }
  @Override
  public void setRecipe(IRecipeLayoutBuilder builder, CobblegenRecipe recipe, IFocusGroup focuses) {
    try {
      ...
      builder.addSlot(RecipeIngredientRole.INPUT, 5, 30).addIngredient(
        ...
        tooltip.add(Component.literal(f1.getAmount() > 999 ? sourceTooltip : flowingTooltip)));
      builder.addSlot(RecipeIngredientRole.INPUT, 45, 30).addIngredient(
        ForgeTypes.FLUID_STACK,
        f2
      ).addTooltipCallback((recipeSlotView, tooltip) ->
        tooltip.add(Component.literal(f2.getAmount() > 999 ? sourceTooltip : flowingTooltip)));
